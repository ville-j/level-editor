  ...
  _createClass(LevelEditor, [{
    ...
    value: function connect(server) {
      ...
      this._socket.on("createvertex", function (v) {
        var polygon = _this5.findPolygon(v.polygonId);

        if (v.afterVertexId) {
          polygon.vertices.splice(_this5.findVertexIndex(v.afterVertexId, polygon) + v.direction, 0, v);
        } else {
          polygon.vertices.push(v);
        }
      });
      ...
      this._socket.on("createpolygon", function (p) {
        _this5.level.polygons.push(p);
      });

      this._socket.on("createobject", function (o) {
        _this5.level.objects.push(o);
      });

      this._socket.on("deletevertex", function (v) {
        var polygon = _this5.findPolygon(v.polygonId);

        polygon.vertices.splice(_this5.findVertexIndex(v.id, polygon), 1);
      });

      this._socket.on("updatevertex", function (v) {
        var vertex = _this5.findVertex(v.id, _this5.findPolygon(v.polygonId));
      ...
      this._socket.on("updateobject", function (o) {
        var obj = _this5.findObject(o.id);
      ...
      this._socket.on("deletepolygon", function (p) {
        _this5.level.polygons.splice(_this5.findPolygonIndex(p), 1);
      });

      this._socket.on("deleteobject", function (o) {
        _this5.level.objects.splice(_this5.findObjectIndex(o), 1);
      });

      this._socket.on("requestlevel", function (clientId) {
        _this5._socket.emit("responselevel", {
          level: _this5.level,
          clientId: clientId
        });
      });
